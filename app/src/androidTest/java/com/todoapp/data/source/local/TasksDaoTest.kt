...
class TasksDaoTest {
    ...
    fun updateTaskAndGetById() = runBlockingTest{
        ...
        loaded.isCompleted = true
        loaded.description = "Updating Description"
        loaded.title = "New Title"

        database.taskDao().updateTask(loaded)


        val updated_task =  database.taskDao().getTaskById(task.id) as Task


        // 3. Check that when you get the task by its ID, it has the updated values.
        assertThat(updated_task , notNullValue())
        assertThat(updated_task.id, `is`(task.id))
        assertThat(updated_task.title, `is`("New Title"))
        assertThat(updated_task.description, `is`("Updating Description"))
        assertThat(updated_task.isCompleted, `is`(true))
    }
}